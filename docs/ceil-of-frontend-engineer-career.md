...
나는 소프트웨어 엔지니어의 분류를 백엔드와 프론트엔드로 분류하는 것이 적절한지 의문이다. 프론트엔드와 백엔드의 구분이 이렇게 주류가 된 건 어떻게 보면 웹 개발에서 관심사의 분리가 중요해진 이후의 일이다. 심지어 프론트엔드 개발자, 백엔드 개발자라고 불리는 이들이 업무에서 웹만 다루는 것도 아니다. 나는 이제 프론트엔드 개발자와 백엔드 개발자가 아닌, 프로덕트 개발자와 [[platform-engineering]]{플랫폼 개발자}로 구분하는 것이 더 적절하다고 생각한다.
